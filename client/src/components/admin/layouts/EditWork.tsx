...
const EditWork = () => {
  ...
  const [work, setWork] = useState<Work>({
    ...
    links: [{ website: "" }],
    badges: [],
    dates: [{ startDate: null, endDate: null }], // Initialize with one date range
  });
    ...
    const fetchWork = async () => {
      try {
        ...
        setWork({
          ...
          dates: response.data.dates || [{ startDate: null, endDate: null }], // Make sure to set dates
        });
      } catch (error) {
  ...
  const handleDateChange = (
    ...
    updatedDates[index][field] = value || null; // Update date or set to null
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    // For initial testing, send only title and description
    const dataToSubmit = {
      title: work.title.trim(),
      description: work.description.trim(),
    };

    console.log("Data to submit:", dataToSubmit); // Log the data to check it
