...
export default function AddWork() {
  ...
  const validateForm = () => {
    ...
    if (
      !workData.title.trim() ||
      workData.title.length < 2 ||
      workData.title.length > 255
    ) {
      newErrors.title =
        "Title is required and must be between 2 and 255 characters";
    }

    if (
      !workData.description.trim() ||
      workData.description.length < 2 ||
      workData.description.length > 255
    ) {
      newErrors.description =
        "Description is required and must be between 2 and 255 characters";
    }
